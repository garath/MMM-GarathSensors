...
Module.register("MMM-GarathSensors", {
	...
	getDom: function () {
	  ...
	  for (const [, sensorDataEntry] of sensorDataEntries) {
		...
		icon.className =
		  "symbol align-right fas " +
		  this.getBatteryLevelIconClass(sensorDataEntry.battery);
		...
		temperatureCell.innerHTML = this.celsiusToFahrenheit(sensorDataEntry.temperatureCelsius) + "°";
	...
	processSensorData: function (sensorData) {
	  ...
	  let indoorSensor = sensorData.find(
		(element) => element.address === this.config.indoorAddress
	  );
	  if (indoorSensor) {
		this.sendNotification(
		  "INDOOR_TEMPERATURE",
		  indoorSensor.temperatureCelsius
		);
	...
	remapAddressName: function (address) {
	  let el = this.config.addressNameMap.find(
